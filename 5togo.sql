...
-- 2. Creare user și alocare spații corespunzătoare
...
-- 3. Comută sesiunea pe userul creat
...
-- Trigger pentru setarea automata a stocului minim (5% din stocul inițial)
...
-- Trigger pentru reducerea stocului și blocarea comenzii dacă stocul scade sub minimul economic
...
UPDATE users
SET password = '$2b$10$QUIHyWRGV/Chfu92tgUs/eDYzkD7zgZ.TCnaBdX.Z8jz17tA4/N72'
...
SELECT
  FLOOR(i.stock_quantity / r.quantity) AS max_produse_posibile
FROM
  ingredients i
  JOIN recipes r ON i.id = r.ingredient_id
WHERE
  r.product_id = 1
ORDER BY max_produse_posibile ASC
...
WHERE
  r.product_id = 5
ORDER BY
  max_produse_posibile ASC
FETCH FIRST 1 ROW ONLY;

SELECT id, name, stock_quantity, minimum_stock FROM ingredients;

SELECT FLOOR(i.stock_quantity / r.quantity) AS max_produse_posibile
FROM ingredients i
JOIN recipes r ON i.id = r.ingredient_id
